...
public class BoardDeadlock : MonoBehaviour
{
    ...
    List<GamePiece> GetMinimumMatches(List<GamePiece> gamePieces, int minForMatch = 2)
    {
        ...
        foreach (var grp in groups)
        {
            ...
            if (grp.Count() >= minForMatch && grp.Key != MatchValue.None && grp.Key != MatchValue.BrokenBarrel && grp.Key != MatchValue.Barrel && grp.Key != MatchValue.Litter)
            {
                matches = grp.ToList();
            }
        }
    ...
    public void HighligthMatch(GamePiece[,] allPieces, int listLength = 3)
    {
        ...
        for (int i = 0; i < width; i++)
        {
            ...
            {
                // if the row or column of GamePieces has an available move, return false; board is not deadlocked
                if (HasMatchAt(allPieces, i, j, listLength, true) || HasMatchAt(allPieces, i, j, listLength, false))
                {
                        return;
    ...
    bool HasMatchAt(GamePiece[,] allPieces, int x, int y, int listLength = 3, bool checkRow = true)
    {
        ...
        if (pieces != null && matches != null)
        {
            ...
            if (matches.Count >= listLength)
            {
                ...
                foreach (GamePiece gp in matches)
                    {
                        
                        if (m_particleManager != null)
                        {
                        lastGlow++;
                            m_particleManager.PossibleMatchFXAt(gp.xIndex, gp.yIndex);
                            quit = true;
    ...
    public void removeMatchGlow()
    {
        if (m_particleManager != null)
        {
            m_particleManager.RemoveMatchFX(lastGlow);
