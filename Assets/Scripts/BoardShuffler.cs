...
public class BoardShuffler : MonoBehaviour
{
    ...
    public List<GamePiece> RemoveNormalPieces(GamePiece[,] allPieces, Tiles2ndLayer[,] tiles2NdLayers, GamePiece[,] allPositions)
    {
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                ...
                if (allPieces[i, j] != null)
                {
                    ...
                    GamePiece piece = allPieces[i, j].GetComponent<GamePiece>();

                    // add to normalPieces list if gamePiece is not Bomb or Collectible
                    if (bomb == null && collectible == null && tiles2NdLayers[i, j] == null)
                    {
                        normalPieces.Add(allPieces[i, j]);
                        allPositions[i, j] = allPieces[i, j];
